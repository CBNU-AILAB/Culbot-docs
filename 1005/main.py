...
neo4j_connection = Neo4jDatabase(
    host=os.environ.get("NEO4J_URL", "neo4j+s://37bea0cc.databases.neo4j.io"),
    user=os.environ.get("NEO4J_USER", "neo4j"),
    password=os.environ.get("NEO4J_PASS", "DWmyEs4rAp1M5jFcrss62xRC3SOlOt_Sm2FVUi3JNsQ"),
    database=os.environ.get("NEO4J_DATABASE", "neo4j"),
)


# Initialize LLM modules
openai_api_key = "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"
        ...
        while True:
            ...
            if data["type"] == "question":
                try:
                    ...
                    results = None
                    try:
                        # cypher 문 생성하고 실행하여 관련된 정보를 찾는다.
                    ...
                    # 관련된 정보를 기반으로 응답을 생성한다.
                    # output = 하나씩 짤려 나오던 답변을 .join을 통해 합쳐서 하나의 문자열로 만들어준다.
                    ...
                    # 응답 생성이 끝나면, 최종 완료 답변을 넣어준다.
                    chatHistory.append({"role": "system", "content": output})

                    # 답변 생성이 끝났다는 것을 알려주기 위해 websocket으로 보낸다.
                    await websocket.send_json(
                        {
                            "type": "end",
                            "output": output,
